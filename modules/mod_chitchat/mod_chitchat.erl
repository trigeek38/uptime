...
event({postback, {newchitchat, Args}, _TriggerId, _TargetId}, Context) ->
    ...
    Props = [{id, Id}],
    {HTML, Context1} = z_template:render_to_iolist("_chitchats.tpl", Props, Context),
    z_render:update("chat-box-container", HTML, Context1);

event({submit, {newchitchat, Args}, _TriggerId, _TargetId}, Context) ->
    {id, Id} = proplists:lookup(id, Args),
    Name = z_context:get_q_validated("chat_name", Context),
    Message = z_context:get_q_validated("chat_message", Context),
